...
{
//	printf("[%s] start\n", __FUNCTION__);
	hmb_debug("Metadata for NVMe SSDs are saving...");
...
int64_t _FTL_READ(struct ssdstate *ssd, int64_t sector_nb, unsigned int length)
{
	...
	while(remain > 0){
		...
		/* SSLAB: n_io_info is not used in this function. */
	...
	HMB_DEBUG__N_READ_PAGES += read_page_nb;
...
int64_t _FTL_WRITE(struct ssdstate *ssd, int64_t sector_nb, unsigned int length)
{
	...
	if(sector_nb + length > SECTOR_NB){
		...
		/* SSLAB: It is not used in this function. */
	...
	HMB_DEBUG__N_WRITTEN_PAGES += write_page_nb;
	return max_need_to_emulate_tt; 
}
