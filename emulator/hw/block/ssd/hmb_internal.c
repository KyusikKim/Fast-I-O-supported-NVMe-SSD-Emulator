...
bool hmb_meta_init(void)
{
	...
	**/
	...
	hmb_meta_get(true)->FASTIO__data_ST_seg_id = -1;
	hmb_meta_get(true)->FASTIO__meta_seg_id = -1;
...
bool hmb_segEnt_init(void)
{
	...
	hmb_debug("  - Host addr      : 0x%llX ~ 0x%llX", \
			HMB_CTRL.segent_host, \
			HMB_CTRL.segent_host + HMB_CTRL.segent_mapped_size -1);
