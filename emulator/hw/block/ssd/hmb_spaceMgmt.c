...
bool hmb_spaceMgmt_table_bm_set_fully(bool enable, uint32_t idx)
{
	...
	else
	{
		...
		hmb_spaceMgmt_table_bm_fill_overflowed( \
				idx, \
				1 << (HMB_SPACEMGMT_CTRL.cache_unit_bits - 9), \
...
bool hmb_spaceMgmt_RC_evict_LRU(uint32_t n_evict, int64_t *expire_time)
{
	...
	for(i=0; i<n_evict; i++)
	{
		...
		/* We cannot distinguish dirty parts in an entry */
		if(entry_r->dirty)
		{
...
void hmb_spaceMgmt_debug_ssd_delay_state(void)
{
	...
	int64_t t_cur = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);

	hmb_debug("#### FLASH DELAY INFO. #### ");
	for(i=0; i<FLASH_NB; i++) 
	{        
		if(chip_next_avail_time[i] - t_cur > 2E9)
		{
	...
	{        
		if(chnl_next_avail_time[i] - t_cur > 2E9)
		{
			hmb_debug("  - %3lu: %lfs. (%lfs.)", i, \
...
uint32_t hmb_spaceMgmt_nlb_to_nlp(uint32_t n_lb)
{
	if(n_lb % (1 << (HMB_SPACEMGMT_CTRL.cache_unit_bits - HMB_SPACEMGMT_CTRL.sector_size_bits)))
	{
		return (n_lb >> (HMB_SPACEMGMT_CTRL.cache_unit_bits - HMB_SPACEMGMT_CTRL.sector_size_bits)) + 1;
	}

	return n_lb >> (HMB_SPACEMGMT_CTRL.cache_unit_bits - HMB_SPACEMGMT_CTRL.sector_size_bits);
}
