...
#include "common.h"
#include <stdbool.h> /* HMB: bool, false, true */
#include "hmb_debug.h" /* hmb_debug() */
...
void INIT_BLOCK_STATE_TABLE(struct ssdstate *ssd)
{
	...
	hmb_debug("Start to restore block state table ...");
	fp = fopen("./data/block_state_table.dat","r");
	if(fp != NULL){
		if(fread(ssd->block_state_table, sizeof(block_state_entry), BLOCK_MAPPING_ENTRY_NB, fp) != BLOCK_MAPPING_ENTRY_NB)
		{
			int i;
			block_state_entry *curr_b_s_entry;

			hmb_debug("  - Failed to restore block state table entries from file.");

			curr_b_s_entry = (block_state_entry*)ssd->block_state_table;
...
void INIT_VALID_ARRAY(struct ssdstate *ssd)
{
	...
	hmb_debug("Start to restore valid array ...");
	fp = fopen("./data/valid_array.dat","r");
	if(fp != NULL) {
		for(i=0;i<BLOCK_MAPPING_ENTRY_NB;i++){
			valid_array = (char*)calloc(PAGE_NB, sizeof(char));
			curr_b_s_entry->valid_array = valid_array;
			if(fread(valid_array, sizeof(char), PAGE_NB, fp) != PAGE_NB)
			{
				int j;

				hmb_debug("  - Failed to restore valid array entry information");
				restore_success = false;

				for(j=0; j<=i; j++)
				{
					memset(((block_state_entry *)block_state_table + j)->valid_array, 0, PAGE_NB);
				}

				curr_b_s_entry += 1;
				for(j=i+1; j<BLOCK_MAPPING_ENTRY_NB; j++)
				{
					valid_array = (char*)calloc(PAGE_NB, sizeof(char));
		...
		hmb_debug("  - Failed to open file for restoration.");
		restore_success = false;
...
void INIT_EMPTY_BLOCK_LIST(struct ssdstate *ssd)
{
	...
	hmb_debug("Start to restore empty block list...");
	fp = fopen("./data/empty_block_list.dat","r");
	if(fp != NULL){
		...
		for(i=0;i<PLANES_PER_FLASH;i++){
			...
			for(j=0;j<FLASH_NB;j++){
				...
				while(k > 0){
					...
					if(fread(curr_entry, sizeof(empty_block_entry), 1, fp) != 1)
					{
						hmb_debug("  - Failed to read data from file.");
						restore_success = false;
					}
...
void INIT_VICTIM_BLOCK_LIST(struct ssdstate *ssd)
{
	...
	hmb_debug("Start to restore victim block list ...");
	fp = fopen("./data/victim_block_list.dat","r");
	if(fp != NULL){
		...
		for(i=0;i<PLANES_PER_FLASH;i++){
			...
			for(j=0;j<FLASH_NB;j++){
				...
				while(k > 0){
					...
					if(fread(curr_entry, sizeof(victim_block_entry), 1, fp) != 1)
					{
						hmb_debug("Failed to read data from file.");
						restore_success = false;
					}
...
void TERM_INVERSE_MAPPING_TABLE(struct ssdstate *ssd)
{
	...
	hmb_debug("Start to store inverse mapping table information...");
	fp = fopen("./data/inverse_mapping.dat", "w");

	if(fp == NULL)
	{
		//printf("ERROR[%s] File open fail\n", __FUNCTION__);
		hmb_debug("  - Failed to open/make to store file.");
		//return;
	}
	...
	else
	{
		...
		if(fwrite(inverse_mapping_table, sizeof(int64_t), PAGE_MAPPING_ENTRY_NB, fp) != PAGE_MAPPING_ENTRY_NB)
		{
			hmb_debug("  - Failed to store data to file.");
			fclose(fp);
			unlink("./data/inverse_mapping.dat");
		}
		else
		{
...
void TERM_BLOCK_STATE_TABLE(struct ssdstate *ssd)
{
	...
	hmb_debug("Start to store block state table ... ");
	fp = fopen("./data/block_state_table.dat","w");
	if(fp == NULL){
		hmb_debug("  - Failed to open/make file");
	//	printf("ERROR[%s] File open fail\n", __FUNCTION__);
	//	return;
	}
	else
	{
		...
		if(fwrite(ssd->block_state_table, sizeof(block_state_entry), BLOCK_MAPPING_ENTRY_NB, fp) != BLOCK_MAPPING_ENTRY_NB)
		{
			hmb_debug("  - Failed to store data");
			fclose(fp);
			unlink("./data/block_state_table.dat");
		}

		hmb_debug("  - All entires were stored.");
		fclose(fp);
	}

	/* Free The inverse block table memory */
	free(ssd->block_state_table);
}
...
void TERM_VALID_ARRAY(struct ssdstate *ssd)
{
	...
	hmb_debug("Start to store valid array ...");
	fp = fopen("./data/valid_array.dat","w");
	if(fp == NULL) {
		hmb_debug("  - Failed to open/make file.");
		//printf("ERROR[%s] File open fail\n", __FUNCTION__);
		return;
	...
	else
	{
		for(i=0;i<BLOCK_MAPPING_ENTRY_NB;i++){
			...
			if(fwrite(valid_array, sizeof(char), PAGE_NB, fp) != PAGE_NB)
			{
				hmb_debug("Failed to write data.");
				fclose(fp);
				unlink("./data/valid_array.dat");
				return;
			}
			curr_b_s_entry += 1;
		}
...
void TERM_EMPTY_BLOCK_LIST(struct ssdstate *ssd)
{
	...
	hmb_debug("Start to store empty block list ...");
	fp = fopen("./data/empty_block_list.dat","w");
	if(fp == NULL){
		hmb_debug("  - Failed to open/make file");
		store_success = false;
		//printf("ERROR[%s] File open fail\n", __FUNCTION__);
	...
	{
		if(fwrite(ssd->empty_block_list,sizeof(empty_block_root),PLANES_PER_FLASH*FLASH_NB, fp) != PLANES_PER_FLASH*FLASH_NB)
		{
			hmb_debug("  - Failed to store data.");
			store_success = false;

			fclose(fp);
			unlink("./data/empty_block_list.dat");
		}
			...
			for(i=0;i<PLANES_PER_FLASH;i++){
				...
				for(j=0;j<FLASH_NB;j++){
					...
					while(k > 0){
						...
						if(fwrite(curr_entry, sizeof(empty_block_entry), 1, fp) != 1)
						{
							store_success = false;
						}
...
void TERM_VICTIM_BLOCK_LIST(struct ssdstate *ssd)
{
	...
	hmb_debug("Start to store victim block list.");
	fp = fopen("./data/victim_block_list.dat","w");
	if(fp == NULL){
		hmb_debug("  - Failed to open/make file.");
		store_success = false;
		//printf("ERROR[%s] File open fail\n", __FUNCTION__);
	...
	{
		if(fwrite(ssd->victim_block_list, sizeof(victim_block_root), PLANES_PER_FLASH*FLASH_NB, fp) != PLANES_PER_FLASH*FLASH_NB)
		{
			hmb_debug("Failed to write data.");
			store_success = false;
			fclose(fp);
			unlink("./data/victim_block_list.dat");
		}
		...
		for(i=0;i<PLANES_PER_FLASH;i++){
			...
			for(j=0;j<FLASH_NB;j++){
				...
				while(k > 0){
					...
					if(fwrite(curr_entry, sizeof(victim_block_entry), 1, fp) != 1)
					{
						store_success = false;
					}
