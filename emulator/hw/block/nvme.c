...
#include "ssd/hmb_spaceMgmt.h" /* HMB: for supporting HMB caching */
#include "ssd/hmb_utils.h" /* HMB: for supporting HMB caching */
#include "ssd/hmb_types.h" /* HMB: for supporting HMB caching */
#include "ssd/hmb_fastIO.h"
#include "ssd/hmb_config.h" /* HMB: for restoring NAND data */
...
void nvme_data_restoration(void)
{
	NvmeCtrl *n = (NvmeCtrl *)HMB_CTRL.parent;
	FILE *f_nand;
	int64_t ret;

	hmb_debug("Start to store NAND data ... ");

	if((f_nand = fopen(HMB_CONFIG__FILENAME__NAND, "w")) == NULL)
	{
		hmb_debug("  - Failed to make file to store NAND data.");
	}
	else
	{
		if((ret = fwrite(n->heap_storage, n->bs_size, 1, f_nand)) != 1)
#if 0
		if((ret = fwrite(n->heap_storage, 1, n->bs_size, f_nand)) != blk_getlength(n->conf.blk))
#endif
		{
			hmb_debug("  - Failed to write data to %s (%ld/%ld).", HMB_CONFIG__FILENAME__NAND, ret, n->bs_size);
		}
		else
		{
			hmb_debug("  - All NAND data were written into file.");
		}
		...
		NvmeCmd *cmd, NvmeRequest *req)
{
	...
	int64_t hmb_dbg_overflowed;

#ifdef HMB_DEBUGGING_TIME
	hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_qemu__sglist, true);
#endif
...
#ifdef HMB_DEBUGGING_TIME
	hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_qemu__sglist, false);
#endif
	if (iov.size == 0) {
...
#ifdef HMB_DEBUGGING_TIME
	hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_qemu__iov, true);
#endif
	if (!QEMU_IS_ALIGNED(iov.size, BDRV_SECTOR_SIZE)) {
		qemu_iovec_discard_back(&iov, QEMU_ALIGN_DOWN(iov.size, BDRV_SECTOR_SIZE));
	}
#ifdef HMB_DEBUGGING_TIME
	hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_qemu__iov, false);
#endif

#if 0
	if(HMB_CTRL.enable_hmb && req->is_write)
	{
		...
		lpn = hmb_spaceMgmt_lba_to_lpn(req->slba);
		nlp = hmb_spaceMgmt_nlb_to_nlp(req->nlb);

#ifdef HMB_DEBUGGING_TIME
		hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__flush, true);
#endif
		if(hmb_spaceMgmt_WB_flush_explicit(nlp, &(n->ssd), &expire_time) == false)
		{
			hmb_spaceMgmt_unlock();

			hmb_debug("Failed to do flushing explicitly.");
#ifdef HMB_DEBUGGING_TIME
			hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb, false);
			hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__flush, false);
#endif
			goto HMB_ORIGINAL_FLOW;
		}
		else
		{
			if(expire_time > 2E9)
			...
			hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__flush, false);
			hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb, true);
			hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__RC_caching, true);
#endif
			if(hmb_spaceMgmt_RC_caching(lpn, nlp, false, &expire_time) == false)
			{
				hmb_spaceMgmt_unlock();

				hmb_debug("Failed to do caching requested data into HMB.");
#ifdef HMB_DEBUGGING_TIME
				hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb, false);
				hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__RC_caching, false);
#endif
				goto HMB_ORIGINAL_FLOW;
			}
			else
			{
				uint64_t iov_idx, iov_offset;
				uint64_t hmb_remainder, hmb_offset;
				...
				for (i = 0; i < nlp; ++i)
				{
...
#ifdef HMB_DEBUGGING_TIME
					hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__table_get_by_lpn, true);
#endif
					if((entry_r = hmb_spaceMgmt_table_get_by_lpn(false, lpn + i)) == NULL)
					{
						hmb_spaceMgmt_unlock();

						hmb_debug("Invalid relationship: cache entries were already made!");
#ifdef HMB_DEBUGGING_TIME
						hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb, false);
						hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__table_get_by_lpn, false);
#endif
						goto HMB_ORIGINAL_FLOW;
					}
					...
					hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__table_get_by_lpn, false);
#endif
					entry_w = hmb_spaceMgmt_table_get_by_idx(true, entry_r->e_own);
					dest_base = hmb_spaceMgmt_mappedAddr_get_by_idx(true, entry_r->e_own);

					if(i == nlp-1)
					{
						uint64_t j;

						hmb_remainder = (req->nlb << HMB_SPACEMGMT_CTRL.sector_size_bits) % HMB_SPACEMGMT_CTRL.cache_unit;
						
						for(j=0; j<hmb_remainder >> HMB_SPACEMGMT_CTRL.sector_size_bits; j++)
						{
							hmb_spaceMgmt_table_bm_set(true, entry_r->e_own, j);
						}
					}
					else
					{
						hmb_remainder = HMB_SPACEMGMT_CTRL.cache_unit;
						hmb_spaceMgmt_table_bm_set_fully(true, entry_r->e_own);
					}
					hmb_offset = 0;
					while(hmb_remainder > 0)
					{
						uint64_t iov_remainder = iov.iov[iov_idx].iov_len - iov_offset;

#if 0
						hmb_debug("BEF: iov_remainder: %lu, hmb_remainder: %lu, iov_offset: %lu, hmb_offset: %lu, iov_idx: %lu, iov_len: %d", \
								iov_remainder, hmb_remainder, iov_offset, hmb_offset, iov_idx, iov.iov[iov_idx].iov_len);
#endif
						if(hmb_remainder > iov_remainder)
						{
							memcpy(dest_base + hmb_offset, \
									iov.iov[iov_idx].iov_base + iov_offset, \
									iov_remainder);
							
							hmb_offset += iov_remainder;
							hmb_remainder -= iov_remainder;

							iov_idx++;
							iov_offset = 0;
#if 0
							hmb_debug("AFT: iov_remainder: %lu, hmb_remainder: %lu, iov_offset: %lu, hmb_offset: %lu, iov_idx: %lu, iov_len: %d", \
									iov_remainder, hmb_remainder, iov_offset, hmb_offset, iov_idx, iov.iov[iov_idx].iov_len);
#endif
						}

						else /* i.e. if (hmb_remainder <= iov_remainder) */
						{
							memcpy(dest_base + hmb_offset, \
									iov.iov[iov_idx].iov_base + iov_offset, \
									hmb_remainder);
							
							iov_offset += hmb_remainder;

							if(iov_offset == iov.iov[iov_idx].iov_len)
							{
								iov_idx++;
								iov_offset = 0;
							}
#if 0
							hmb_debug("AFT: iov_remainder: %lu, hmb_remainder: %lu, iov_offset: %lu, hmb_offset: %lu, iov_idx: %lu", \
									iov_remainder, hmb_remainder, iov_offset, hmb_offset, iov_idx);
#endif
					...
					if(entry_r->urgency == HMB_SPACEMGMT_UL_DISABLED)
					{
...
#endif
						hmb_spaceMgmt_shared_set_dirty(true, entry_r->e_own);
						hmb_spaceMgmt_table_nDirty_inc(true);

						entry_w->urgency = HMB_SPACEMGMT_UL_URGENT;

#ifdef HMB_DEBUGGING_TIME
						hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__insert, true);
#endif
						if(hmb_spaceMgmt_WB_sorted_insert_tail(entry_r->urgency, entry_r->e_own) == false)
						{
							hmb_spaceMgmt_unlock();

							hmb_debug("Failed to insert sorted entry for WB");
#ifdef HMB_DEBUGGING_TIME
							hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb, false);
							hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__insert, false);
#endif
							goto HMB_ORIGINAL_FLOW;
						}
#ifdef HMB_DEBUGGING_TIME
					...
					else
					{
...
#endif
						if(hmb_spaceMgmt_WB_sorted_delete(entry_r->urgency, entry_r->e_own) == false)
						{
							hmb_spaceMgmt_unlock();
...
#endif
							hmb_debug("Failed to delete WB sorted entry.");
							goto HMB_ORIGINAL_FLOW;
						}
						...
						if(hmb_spaceMgmt_WB_sorted_insert_tail(entry_r->urgency, entry_r->e_own) == false)
						{
							...
							hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__insert, false);
#endif
							hmb_debug("Failed to re-insert the WB sorted entry.");
							goto HMB_ORIGINAL_FLOW;
						}
#ifdef HMB_DEBUGGING_TIME
						hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__insert, false);
...
#ifdef HMB_DEBUGGING_TIME
	hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_qemu__memcpy, true);
#endif
#endif
	hs_oft = req->data_offset;
	if (req->is_write) {
		// iov -> heap storage
...
#ifdef HMB_DEBUGGING_TIME
	hmb_timeDbg_record(&HMB_DEBUG_TIME_PER_REQUEST.of_qemu__memcpy, false);
#endif
...
HMB_COMPLETE:
	...
	qemu_iovec_destroy(&iov); /* Fixed bug to free dynamically allocated memory. */

	if((hmb_dbg_overflowed = req->expire_time - qemu_clock_get_ns(QEMU_CLOCK_REALTIME)) <= 0)
	{
		HMB_DEBUG__SUM_OVERFLOWED_TIME += (uint64_t)(-hmb_dbg_overflowed);
		HMB_DEBUG__N_OVERFLOWED++;

#ifdef HMB_DEBUGGING_TIME
#if 0
		hmb_debug("<%s> [OVERFLOWED - %6lu/%6lu] SLBA: 0x%7lX, #LBs: %3lu, T-EXP: %9lu, T-OF: %ld, T-FEMU: %lu, T-HMB: %lu, T-FLUSH: %lu, T-RC-caching: %lu, T-table-get-by-lpn: %lu, T-INSERT: %lu, T-DELETE: %lu, T-BM-op: %lu, T-memcpy: %lu", \
				req->is_write?"W":"R", \
				HMB_DEBUG__N_OVERFLOWED, HMB_DEBUG__N_REQUESTS, \
				req->slba, req->nlb, req->expire_time, \
				-hmb_dbg_overflowed, \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_femu), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__flush), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__RC_caching), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__table_get_by_lpn), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__insert), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__delete), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__bm_op), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb__memcpy));
#endif
		hmb_debug("<%s> [OVERFLOWED - %6lu/%6lu] SLBA: 0x%7lX, #LBs: %3lu, T-EXP: %9lu, T-OF: %ld, T-FEMU: %lu, T-HMB: %lu, T-sglist: %lu, T-memcpy: %lu", \
				req->is_write?"W":"R", \
				HMB_DEBUG__N_OVERFLOWED, HMB_DEBUG__N_REQUESTS, \
				req->slba, req->nlb, req->expire_time - HMB_SPACEMGMT_BASE_TIME, \
				-hmb_dbg_overflowed, \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_femu), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_qemu__sglist), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_qemu__memcpy));
#endif
	}
	...
	{
		hmb_debug("<%s>    [NORMAL - %6lu/%6lu] SLBA: 0x%7lX, #LBs: %3lu, T-EXP: %9lu, T-FEMU: %lu, T-HMB: %lu, T-sglist: %lu, T-memcpy: %lu", \
				req->is_write?"W":"R", \
				HMB_DEBUG__N_REQUESTS - HMB_DEBUG__N_OVERFLOWED, HMB_DEBUG__N_REQUESTS, \
				req->slba, req->nlb, req->expire_time - HMB_SPACEMGMT_BASE_TIME, \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_femu), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_hmb), \
				hmb_timeDbg_get_time_ns(&HMB_DEBUG_TIME_PER_REQUEST.of_qemu__sglist), \
...
static void nvme_post_cqes_io(void *opaque)
{
    ...
    QTAILQ_FOREACH_SAFE(req, &cq->req_list, entry, next) {
        ...
        */
		...
		/* SSLAB: to free unused memory dynamically allocated in previous */
		if (req->qsg.nsg) {
    ...
    NvmeRequest *req)
{
	...
	HMB_SPACEMGMT_BASE_TIME = qemu_clock_get_ns(QEMU_CLOCK_REALTIME); 

#ifdef HMB_DEBUGGING_TIME
	HMB_DEBUG_TIME_PER_REQUEST.of_qemu__iov.t_acc_ns = 0;
	HMB_DEBUG_TIME_PER_REQUEST.of_qemu__sglist.t_acc_ns = 0;
	HMB_DEBUG_TIME_PER_REQUEST.of_qemu__memcpy.t_acc_ns = 0;

	HMB_DEBUG_TIME_PER_REQUEST.of_femu.t_acc_ns = 0;
	HMB_DEBUG_TIME_PER_REQUEST.of_femu__get_empty_block.t_acc_ns = 0;

	HMB_DEBUG_TIME_PER_REQUEST.of_hmb.t_acc_ns = 0;
	HMB_DEBUG_TIME_PER_REQUEST.of_hmb__flush.t_acc_ns = 0;
	HMB_DEBUG_TIME_PER_REQUEST.of_hmb__RC_caching.t_acc_ns = 0;
	HMB_DEBUG_TIME_PER_REQUEST.of_hmb__table_get_by_lpn.t_acc_ns = 0;
	HMB_DEBUG_TIME_PER_REQUEST.of_hmb__insert.t_acc_ns = 0;
	HMB_DEBUG_TIME_PER_REQUEST.of_hmb__delete.t_acc_ns = 0;
	HMB_DEBUG_TIME_PER_REQUEST.of_hmb__bm_op.t_acc_ns = 0;
	HMB_DEBUG_TIME_PER_REQUEST.of_hmb__memcpy.t_acc_ns = 0;
	...
	req->expire_time = HMB_SPACEMGMT_BASE_TIME; // + 200000 - overhead;

    if (req->is_write) {
		HMB_DEBUG__N_WRITE_REQUESTED_SECTORS += nlb;

        if (n->femu_mode == FEMU_BLACKBOX_MODE)
		{
#if 0
			if(HMB_CTRL.enable_hmb == false)
			{
#endif
				int64_t ret;
#ifdef HMB_DEBUGGING_TIME
				...
				req->expire_time += ret;
				//hmb_debug("<W> Expire time: %.4lf (HMB: disabled)", (double)ret / 1E9);
				if(ret > 2E9) 
				{    
					hmb_spaceMgmt_debug_ssd_delay_state();
				}    
	...
	{
		HMB_DEBUG__N_READ_REQUESTED_SECTORS += nlb;
        ...
        if (n->femu_mode == FEMU_BLACKBOX_MODE)
		{
			...
			req->expire_time += ret;
			//hmb_debug("<R> Expire time: %.4lf, LBA: 0x%010lX, #LB: %u", (double)ret / 1E9, slba, nlb);
			...
			if(HMB_CTRL.enable_hmb)
			{
				for(i=0; i<nlb; i++)
				{
					uint64_t lba_cur = slba + i;
					HmbSharedEnt *entry_r;

					entry_r = hmb_spaceMgmt_table_get_by_lba(false, lba_cur);
					if(entry_r != NULL)
					{
						hmb_debug("LBA: 0x%010lX(0x%010lX), U: %1lu, D: %1lu, UL: %2lu", \
								lba_cur, entry_r->lba, entry_r->usable, entry_r->dirty, entry_r->urgency);
					}
				}
			}
#endif
...
static uint16_t nvme_io_cmd(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
	...
	static uint64_t hmb_n_called_flush = 0;
    ...
    case NVME_CMD_FLUSH:
		{
			...
			//hmb_debug("NVMe Flush command was requested! (# calls: %lu)", ++hmb_n_called_flush);
			if(HMB_SPACEMGMT_CTRL.inited)
			{
				t_dbg = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);
				if(nvme_flush_from_hmb(req) == false)
				{
					hmb_debug("NVMe Flush command was requested, but it was failed to process successfully.");
				}
				t_dbg = qemu_clock_get_ns(QEMU_CLOCK_REALTIME) - t_dbg;
				hmb_debug(" - Elapsed time: %lf sec.", ((double)t_dbg)/1E9);
			}
		}
		return NVME_SUCCESS;
//		return NVME_NO_COMPLETE;
    ...
    case NVME_CMD_WRITE_ZEROS:
		hmb_debug("NVME_CMD_WRITE_ZEROES");
    ...
    case NVME_CMD_WRITE_UNCOR:
		hmb_debug("NVME_CMD_WRITE_UNCORRECTABLE");
...
static uint16_t nvme_create_sq(NvmeCtrl *n, NvmeCmd *cmd)
{
	...
	hmb_debug("A SQ %u was created!", sqid);
...
static void nvme_partition_ns(NvmeNamespace *ns, uint8_t lba_idx)
{
    ...
    */
	...
	hmb_debug("id_ns->nuse = %llu, id_ns->ncap = %llu, and id_ns->nsze = %llu", \
...
static uint16_t nvme_fastIO(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
	HmbFastIOMeta *meta_r;
	void *data_inHmb_w, *data_inHmb_r;
	struct ssdstate *ssd = &(n->ssd);
	int64_t t_expire;
#if 0
	int64_t ret;
#endif
	void *heap = n->heap_storage;
	void *heap_target;

	NvmeRwCmd *cmd_io = (NvmeRwCmd *)cmd;
	uint64_t lba, nlb;
	bool is_write;
	...
	*/
	...
	t_expire = HMB_SPACEMGMT_BASE_TIME = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);

	meta_r = hmb_fastIO_get_meta(false);

	lba = meta_r->lba;
	nlb = meta_r->nlb;
	is_write = (meta_r->is_write) ? true : false;

	/*
    if ((lba + nlb > le64_to_cpu(n->namespaces[nsid-1].id_ns.nsze)) || \
			((nlb << 9) > HMB_FASTIO_CTRL.max_data_size_bytes))
	*/
    if ((lba + nlb > le64_to_cpu(n->namespaces[0].id_ns.nsze)) || \
			((nlb << 9) > HMB_FASTIO_CTRL.max_data_size_bytes))
	{
        return NVME_LBA_RANGE;
    }

	heap_target = (void *)((uint64_t)heap + 512 * lba);

	if(is_write)
	{   
		uint64_t off_start;
#if 0
		ret = SSD_WRITE(ssd, nlb, lba);
#endif
		t_expire += SSD_WRITE(ssd, nlb, lba);

		for(off_start = 0; off_start < (nlb << 9); off_start += HMB_FASTIO_CTRL.data_ST_unit)
		{
			void *dest = heap_target + off_start;
			void *src = hmb_fastIO_get_data(false, off_start);
			uint64_t len;
			
			if(off_start + HMB_FASTIO_CTRL.data_ST_unit >= (nlb << 9))
			{
				len = (nlb << 9) - off_start;
			}
			...
			{
				len = HMB_FASTIO_CTRL.data_ST_unit;
			}
			memcpy(dest, src, len);
		}
	}   
	...
	{   
		uint64_t off_start;
#if 0
		ret = SSD_READ(ssd, nlb, lba);
#endif
//#if 0
		t_expire += SSD_READ(ssd, nlb, lba);
//#endif
		for(off_start = 0; off_start < (nlb << 9); off_start += HMB_FASTIO_CTRL.data_ST_unit)
		{
			void *dest = hmb_fastIO_get_data(true, off_start);
			void *src = heap_target + off_start;
			uint64_t len;
...
#if 0
	t_expire += ret;
	hmb_debug("<%s> from 0x%lx, +%lu, Delayed time: %ld", \
			is_write?"WRITE":"READ", \
			lba, \
			nlb, \
			ret);
	...
	/* Busy waiting... */
	while(t_expire > qemu_clock_get_ns(QEMU_CLOCK_REALTIME)); 
...
static uint16_t nvme_admin_cmd(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
	...
	case NVME_ADM_CMD_FAST_IO_IN_HMB:
		return nvme_fastIO(n, cmd, req);
...
static void nvme_process_sq_admin(void *opaque)
{
    ...
    while (!(nvme_sq_empty(sq) || QTAILQ_EMPTY(&sq->req_list)) &&
		...
		//hmb_debug("Processed in Admin queue: %4d/%4d", processed, sq->arb_burst);
    }
...
static void nvme_process_sq_io(void *opaque)
{
    ...
    while (!(nvme_sq_empty(sq) || QTAILQ_EMPTY(&sq->req_list)) &&
		...
		//hmb_debug("Processed in IOCMD queue: %4d/%4d", processed, sq->arb_burst);
    }
    ...
    unsigned size)
{
    ...
    case 0x14:
        ...
        if (NVME_CC_SHN(data) && !(NVME_CC_SHN(n->bar.cc))) {
				...
				/* HMB: controller termination point */
...
static void nvme_init_namespaces(NvmeCtrl *n)
{
    ...
    for (i = 0; i < n->num_namespaces; i++) {
		...
		hmb_debug("blks = %llu", blks);
        id_ns->nuse = id_ns->ncap = id_ns->nsze = cpu_to_le64(blks);
		hmb_debug("id_ns->nuse = %llu, id_ns->ncap = %llu, and id_ns->nsze = %llu", \
				id_ns->nuse, id_ns->ncap, id_ns->nsze);
		//hmb_spaceMgmt_init_cache_unit(ns->id_ns.lbaf[lba_index].ds);
		hmb_spaceMgmt_init_cache_unit(12);
		//hmb_spaceMgmt_init_cache_unit(9);
		HMB_SPACEMGMT_CTRL.sector_size_bits = ns->id_ns.lbaf[lba_index].ds;
...
static void nvme_init_ctrl(NvmeCtrl *n)
{
    ...
    n->features.arbitration        = 0x1f0f0702; /* Set default AB to 4 */
    //n->features.arbitration        = 0x1f0f0704; /* Set default AB to 16 */
	...
	n->features.host_memory_buffer = 0x0; /* HMB: It is initialized by NVMe Set Feature command. */
...
static int nvme_init(PCIDevice *pci_dev)
{
	...
	hmb_debug("bs_size = %lld B = %.3lfMB", bs_size, (double)bs_size / 1024 / 1024);
	...
	/* HMB: prevent page swap */
	if(mlock(n->heap_storage, bs_size))
	{
		hmb_debug("Failed to prevent page swap for \"heap_storage\"");
		return -1;
	}
	n->bs_size = bs_size;

	hmb_debug("Start to restoring NAND data ... ");

	if((f_nand = fopen(HMB_CONFIG__FILENAME__NAND, "r")) == NULL)
	{
		hmb_debug("  - There are no data for NAND. Data will be filled to zero.");
	}
	else
	{
		hmb_debug("  - File for NAND data was opened.");
		if(fread(n->heap_storage, bs_size, 1, f_nand) != 1)
		{
			hmb_debug("  - Failed to read data from %s. Data will be filled to zero.", HMB_CONFIG__FILENAME__NAND);
			memset(n->heap_storage, 0, bs_size);
		}
	...
	hmb_debug("ns_size = %lld B = %.3lfMB", n->ns_size, (double)n->ns_size / 1024 / 1024);
	...
	atexit(nvme_data_restoration);/* HMB: data will be saved after termination */
...
static void nvme_exit(PCIDevice *pci_dev)
{
	...
	if(munlock(n->heap_storage, blk_getlength(n->conf.blk)))
	{
		hmb_debug("Failed to release prevention of page swap.");
	}
...
static Property nvme_props[] = {
    ...
    DEFINE_PROP_UINT32("entries", NvmeCtrl, max_q_ents, 0xfffe), /* HMB: 2048->0x7ff, 16384->0x3fff, 65532->0xfffe (i.e. 0-based) */
    ...
    DEFINE_PROP_UINT8("vwc", NvmeCtrl, vwc, 1), /* HMB: to enable volatile write cache */
    ...
    DEFINE_PROP_UINT8("lba_index", NvmeCtrl, lba_index, 0), /* HMB: 0->512B, 1->1024B, 2->2048B, ... (x2) */
