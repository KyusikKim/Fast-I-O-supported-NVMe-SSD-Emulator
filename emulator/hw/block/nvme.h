...
/** [1] HMB: Header files for supporting HMB **/
#include "ssd/hmb.h"
#include "ssd/hmb_internal.h"
/** [1] end **/
...
enum NvmeAdminCommands {
	...
	NVME_ADM_CMD_FAST_IO_IN_HMB = 0xc0, /* HMB: */
...
typedef struct NvmeIdCtrl {
	...
	uint8_t     elpe;
	uint8_t     npss;
	/** HMB: New elements for HMB **/
	uint8_t     rsvd271[8];     /* 264~271: not used area */
	uint32_t    hmpre;          /* 272~275: HMB Preferred Size */
	uint32_t    hmmin;          /* 276~279: HMB Minimum Size */
	uint8_t     rsvd511[232];   /* 280~511: not used area */
/*  uint8_t     rsvd511[248]; */
	/** HMB: end **/
    uint8_t     sqes;
    uint8_t     cqes;
...
enum NvmeIdCtrlOacs {
    ...
    //NVME_OACS_DBBUF     = 1 << 7, /* HMB: for using Kernel 4.13.10 */
    NVME_OACS_DBBUF     = 1 << 8,
};
...
typedef struct NvmeFeatureVal {
	...
	uint32_t    host_memory_buffer; /* HMB: for supporting HMB */
...
enum NvmeFeatureIds {
	...
	NVME_HOST_MEMORY_BUFFER         = 0xd, /* HMB: for supporting Host Memory Buffer */
...
typedef struct NvmeCtrl {
	...
	/* HMB: structrue for managing HMB */
	HmbCtrl *hmb_ctrl;
	int64_t bs_size;
} NvmeCtrl;
